...
namespace fuzzy
{
	...
	* @param result_aggregator The inferred fuzzy output to scale to.
	* @return The fuzzy scaled inference.
	*/
	template <class V, class M, template <typename T, typename Alloc = std::allocator<T>> class Container, class Tnorm>
		requires fuzzy::numeric<V> && std::floating_point<M> && tnorm_type<Tnorm>
	constexpr typename scaling_inference<V, M, Container, Tnorm>::set_type scaling_inference<V, M, Container, Tnorm>::apply(V value, set_type const& result_aggregator) const
	{
		using math::promote;
		...
		V const rmin = result_aggregator.front().value();
		V const rmax = result_aggregator.back().value();
		...
		M const range_membership = Tnorm::apply(antecedent_->membership(value), result_aggregator.membership(range_value));
